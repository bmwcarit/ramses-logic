...
def check_header_guards(filename, file_contents):
    ...
    if len(pragma_match) > 1:
        log_warning("check_header_guards", filename, 1, "more than one occurance of header guard found")
...
\tGives warnings if header guards does not exist in header files. Header files must have a single '#pragma once' guard directly after the license.
""")
        exit(0)
